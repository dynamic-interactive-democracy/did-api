...
module.exports = function(req, res) {
    ...
    req.pgdb.query("SELECT data FROM circles WHERE id=$1", [req.params.id], (error, result) => {
        if(!result.rowCount) {
        ...
        req.pgdb.query("UPDATE circles SET data=$1 WHERE id=$2 RETURNING id, data", [updatedCircle, req.params.id], (error, result) => {
            if(error) {

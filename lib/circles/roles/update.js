...
module.exports = function(req, res) {
    const cleanRoleParams = cleanParams(req.body, [
        "title", "effects", "term", "areaOfResponsibility", "desiredCharacteristics"
    ]);
...
function updateRole(pgdb, id, params, callback) {
    getRole(pgdb, id, (error, role) => {
        ...
        pgdb.query("UPDATE roles SET data=$1 WHERE id=$2 RETURNING id, data", [updatedRole, updatedRole.canonicalRoleId], (error, result) => {
            if(error) {
                return callback(error);
            }
